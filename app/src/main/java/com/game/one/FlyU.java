...
public class FlyU extends Sprite
{
    public String flyId = "";
    ...
    public FlyU(GameView view, Context context, String id, boolean spitOut)
    {
        super(view, context);
    ...
    public boolean isTouching(int x2, int y2)
    {
        if (x2 > x && x2 < x + width && y2 > y && y2 < y + height)
        {
            if (v.getFlyE().getBeEaten() == false
                    ...
                    && v.getFlyA().getBeEaten() == false && eat == false)
            {
                Game.theGame.setFlyId("U");
